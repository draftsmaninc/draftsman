    ...
    */
    ...
    public function getModelShow($model): ?\stdClass
    {
        ...
        if ($shown || isset($fallback)) {
            ...
            // Schema drift, surfaced: model:show happily exits 0 with zero
            // attributes for a model whose table was never migrated (BHE's
            // Sponsor). Without the flag the graph shows an inexplicable
            // ghost — no fields, and no edges either, since edges need
            // endpoint columns to attach to.
            if (! Schema::hasTable($data->table)) {
                $data->missing_table = true;
            }
    ...
    protected function pivotTableShows(array $shows): array
    {
        ...
        foreach ($tables as $table => $info) {
            // A traversed-but-unmigrated table (BHE's riderables) still emits,
            // flagged — schema drift is surfaced, not silently skipped.
            $tableExists = Schema::hasTable($table);
            $class = min(array_keys($info['ids']));

            $uniques = ! $tableExists ? collect() : collect(Schema::getIndexes($table))
                ->filter(fn ($i) => ($i['unique'] ?? false) && count($i['columns']) === 1)
                ->map(fn ($i) => $i['columns'][0])
                ->flip();
            $attributes = collect($tableExists ? Schema::getColumns($table) : [])->map(fn ($c) => (object) [
                'name' => $c['name'],
                'type' => $c['type'],
                'increments' => (bool) ($c['auto_increment'] ?? false),
                'nullable' => (bool) ($c['nullable'] ?? false),
            ...
            $pivots[] = (object) [
                ...
                ...($tableExists ? [] : ['missing_table' => true]),
            ];
        }
